...
X = genere_Gibbs_proba(m + 2, n + 2, classes, proba_Markov_field, 50)
...
# list_HMF = {}
# for i in range(1, 10):
#     list_HMF[f'X_{5*i}'] = genere_Gibbs_proba(m+2, n+2, classes, proba_Markov_field, 5*i)
#     list_HMF[f'X_{5*i}'] = list_HMF[f'X_{5*i}'].astype('uint8')
#     list_HMF[f'X_{5*i}'] = redecoupe_image(list_HMF[f'X_{5*i}'])
    
# list_HMF['X_200'] = genere_Gibbs_proba(m+2, n+2, classes, proba_Markov_field, 200)
