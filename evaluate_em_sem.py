...
def main(em=True, sem=True, noise=1, init=1):
    ...
    # EM error rate for 100 iterations
    if em:
        errors_em = []
        for i in range(100):
            ...
            error_em = ((abs(p1_em - p1)/p1) + (abs(p2_em - p2)/p2) + (abs(m1_em - m1)/m1) + (abs(sig1_em - sig1)/sig1) + (abs(m2_em - m2)/m2) + (abs(sig2_em - sig2)/sig2))/6
            errors_em.append(error_em)
        print(np.array(errors_em).mean())
    
    # SEM error rate for 100 iterations
    if sem:
        errors_sem = []
        for i in range(100):
            p1_sem, p2_sem, m1_sem, sig1_sem, m2_sem, sig2_sem, dic_sem = calc_SEM(Y, m, n, cl1, cl2, p10, p20, m10, sig10, m20, sig20, 100) # SEM algorithm estimates
        ...
        print(np.array(errors_sem).mean())
